...
class UserViewTestCase(TestCase):
    ...
    def test_display_new_post_form(self):
        """Tests display new post form"""

        with self.client as c:
            resp = c.get(f"/users/{self.user_id}/posts/new")
            html = resp.get_data(as_text=True)
            self.assertIn("NEW POST FORM DO NOT DELETE", html)

    def test_process_new_post_form(self):
        """Tests processing of new post form"""

        with self.client as c:
            resp = c.post(
                f"users/{self.user_id}/posts/new",
                data={
                    "title": "test title process post form",
                    "content": "test content process post form",
            ...
            html = resp.get_data(as_text=True)
            # add test for errors after adding error handling
            self.assertIn("test title process post form", html)
